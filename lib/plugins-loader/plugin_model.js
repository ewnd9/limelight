...
Plugin.load = function (pluginDir, callback) {
  ...
  fs.readFile(manifestPath, {encoding: 'utf8'}, function (err, manifest) {
    ...
    try {
      ...
      if (Array.isArray(manifest.os) && manifest.os.length === 1 && manifest.os[0] === 'darwin') {
        return callback()
      }
...
var runNode = function (command, item, query, callback) {
  var args = _.slice(arguments, 1)
  require(this.mainFilePath)[command].apply(null, args)
}
...
Plugin.prototype.list = function (query, callback) {
  ...
  if (/\.js$/.test(this.mainFilePath)) runNode.call(this, 'list', query, callback)
...
Plugin.prototype.get = function (item, query, callback) {
  if (/\.js$/.test(this.mainFilePath)) runNode.call(this, 'get', item, query, callback)
  else return runPython.call(this, query, 'results', callback)
}

Plugin.prototype.run = function (item, query, callback) {
  callback = runTransform(callback).bind(this)
  if (/\.js$/.test(this.mainFilePath)) runNode.call(this, 'run', item, query, callback)
